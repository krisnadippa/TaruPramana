...
export default function () {
  const idResep = Math.floor(Math.random() * 5) + 1;
  const selectParams = '*,cara_pemakaian(*),tutorial(*),bahan_resep(*),pivot_resep_penyakit(penyakit(*)),pivot_resep_tanaman(tanaman(*))';
  const url = `${SUPABASE_URL}/rest/v1/resep?select=${encodeURIComponent(selectParams)}&id_resep=eq.${idResep}`;
